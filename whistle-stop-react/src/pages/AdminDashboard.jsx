        ...
        <main className="flex-1 p-4 md:p-6 overflow-auto pb-20 md:pb-6">
          ...
          {adminTab === 'dashboard' && (
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="glass-card p-5"><h3 className="text-xs font-bold tracking-wider text-gray-400 mb-3">PEAK/VALLEY HOURS</h3><MiniBarChart data={peakHoursData} height={120} /></div>
              <div className="glass-card p-5"><h3 className="text-xs font-bold tracking-wider text-gray-400 mb-3">SCHEDULED APPOINTMENTS</h3><div className="space-y-2 max-h-44 overflow-y-auto">{appointments.map(apt => (<div key={apt.id} className="flex items-center justify-between text-sm py-2 border-b border-white/5 last:border-0"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-whistle-grey rounded-full flex items-center justify-center"><Icons.User className="w-4 h-4" /></div><div><p className="font-medium">{apt.barber}</p><p className="text-xs text-gray-500">{apt.service}</p></div></div><span className="text-whistle-crimson font-medium">{apt.time}</span></div>))}</div></div>
